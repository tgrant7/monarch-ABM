...
August 2021 - I uploaded an updated version of the model that can populate irregularly shaped shapefiles. The lat/long coordinates for a bounding box must be supplied. Random x and y (lat and long) coords are chosen for monarch agents, and each one is tested to see if it falls in shapefile, until the number of agents specified is reached. Random x,y coords that fall outside of the shapefile are discarded. 
